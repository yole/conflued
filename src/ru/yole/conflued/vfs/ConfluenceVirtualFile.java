    ...
    public OutputStream getOutputStream(Object requestor, long newModificationStamp, long newTimeStamp) throws IOException {
        return new ByteArrayOutputStream() {
            ...
            public void close() throws IOException {
                ...
                final String content = toString(CharsetToolkit.UTF8);
                ...
                if (!client.isOffline()) {
                    client.updatePage(myPage, content).doWhenDone(new Runnable() {
                        public void run() {
                            myPage.setLocallyModified(false);
                            try {
                                PageContentStore.getInstance().storeContent(myPage.getId(), myPage.getVersion(), content);
                            } catch (IOException e) {
                                // ignore?
                            }
